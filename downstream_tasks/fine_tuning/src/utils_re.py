...
def evaluate_sent_label(sent, counts, label='located_at'):
    correct_ner = set()
    # Entities.
    counts["ner_gold"] += len(sent.ner)
    counts["ner_predicted"] += len(sent.predicted_ner)
    ...
    # Relations.
    relations = [rel for rel in sent.relations if rel.label==label]
    predicted_relations = [rel for rel in sent.predicted_relations if rel.label==label]
    
    counts["relations_gold"] += len(relations)
    counts["relations_predicted"] += len(predicted_relations)
    for prediction in predicted_relations:
        if any([prediction == actual for actual in relations]):
            counts["relations_matched"] += 1
            if (prediction.pair[0] in correct_ner) and (prediction.pair[1] in correct_ner):
                counts["strict_relations_matched"] += 1
...
def eval_data_macro(data):
    macro = []
    for label in ['located_at', 'modify', 'suggestive_of']:
        counts = Counter()

        for doc in data:
            for sent in doc:
                counts = evaluate_sent_label(sent, counts, label)

        scores_relations = compute_f1(
            counts["relations_predicted"], counts["relations_gold"], counts["strict_relations_matched"])

        f1 = scores_relations['f1']
        macro.append(f1)
        
    return np.array(macro)


def evaluate_labeler(json_file1, json_file2):

    data1 = Dataset(json_file=json_file1)
    data2 = Dataset(json_file=json_file2)

    # micro
    micro1 = evaluate_predictions(data1)['strict_relation']['f1']
    micro2 = evaluate_predictions(data2)['strict_relation']['f1']

    micro = np.mean([micro1, micro2])

    # macro
    macro1 = eval_data_macro(data1)
    macro2 = eval_data_macro(data2)

    macro = np.mean([macro1.mean(), macro2.mean()])
