...
import transformers
from utils_qa.trainer_qa import QuestionAnsweringTrainer
...
from utils_qa.utils_qa import postprocess_qa_predictions
from utils import update_output_dir
...
class DataTrainingArguments:
    ...
    dataset_dir: str = field(
        default=None, metadata={"help": "Dir to raw datasets: splitted json files for qa datasets."}
    )
    dataset_cache_dir:  Optional[str] = field(
        default=None, metadata={"help": "Dir to cached hf datasets."}
    )

    remove_dup: bool = field(
        default=False, metadata={"help": "Remove duplicated answers"}
    )
...
def parse_fold(data, to_pandas=True, sample_size=None):
    ...
    def _fix_ans(answer): return {k:[v] for k,v in answer.items() if k in ['text', 'answer_start']}
...
def prepare_qa_datasets(dataset_dir, subset_name, REMOVE_DUP=False):
    
    folds = ['train', 'dev', 'test']
    dsets = []
    
    for fold in folds:
        json_path = os.path.join(dataset_dir, subset_name, f'{fold}.json')
        json_data = json.load(open(json_path))

        if subset_name == 'radqa':
...
def main():
    ...
    if data_args.dataset_name == 'squad':
        ...
        # Load emrQA subsets or radqa
        dataset_name = data_args.dataset_name

        if data_args.dataset_cache_dir is not None:
            # cache data
            cache_path = os.path.join(data_args.dataset_cache_dir, dataset_name)
            try:
                raw_datasets = datasets.load_from_disk(dataset_path=cache_path)
            except:
                raw_datasets = prepare_qa_datasets(data_args.dataset_dir, dataset_name, REMOVE_DUP=data_args.remove_dup)
        ...
        logging.info(f"Loaded emrQA subset on {dataset_name}")
    ...
    training_args = update_output_dir(training_args, model_args, data_args, 'QA')
