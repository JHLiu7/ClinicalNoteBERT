...
def test():
    model_path = 'CKPT/base_basic-4096-20k' 

    ROOT_DIR='/scratch/punim1362/jinghuil1/TmpCBERT/'
    ...
    RAW_TEXT_DIR = '/scratch/punim1362/jinghuil1/TmpCBERT/data/PT_v2/RAW_TEXT'
    bsize=64
    epoch=5_000
    NUM_SEQS=bsize*epoch

    LINE_TYPE='sentence'
    cache_dir=os.path.join(ROOT_DIR, 'cache')
    seed=42

    dset = get_tokenized_lines(
        tokenizer, RAW_TEXT_DIR, NUM_SEQS, LINE_TYPE, cache_dir, seed
    )

    logging.info(f'obtained {len(dset)} lines ()')
    

def get_tokenized_lines(tokenizer, RAW_TEXT_DIR, NUM_SEQS, LINE_TYPE, cache_dir, seed):
    ...
    if LINE_TYPE == 'sentence':
        min_len = 16
        max_len = 128
        FOLD = 5 # keep extra lines for processing
    ...
    for category in NOTE_CATEGORIES:
        raw_text_path = os.path.join(RAW_TEXT_DIR, category, f"{LINE_TYPE}.txt")
        dset = load_dataset('text', data_files=raw_text_path, cache_dir=cache_dir)['train']

        all_datasets.append(dset)

    # sample lines
    raw_dataset = concatenate_datasets(all_datasets).shuffle(seed=seed)
    raw_dataset = raw_dataset.select(range(NUM_SEQS*FOLD))

    logging.info(f'Ready to tokenize {len(raw_dataset)} raw {LINE_TYPE} lines')
    
    # tokenize
    tok = TokenizeEngine(tokenizer)

    tokenized_dataset = raw_dataset.map(
        tok, batched=True, num_proc=N_JOBS,
        remove_columns=['text'], load_from_cache_file=False,
        desc="Running tokenizer on every text in dataset",
    )
    logging.info(f'Tokenized {len(tokenized_dataset)} {LINE_TYPE} lines')

    # process and group lines if necessary
    ...
    processed_dataset = tokenized_dataset.map(
        proc, batched=True, num_proc=N_JOBS,
        remove_columns=['length'], load_from_cache_file=False,
        desc="Processing lines",
    )

    logging.info(f'Processed {len(processed_dataset)} {LINE_TYPE} lines')
